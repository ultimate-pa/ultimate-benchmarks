...
modifies ;
{
  ...
  loc2_1:
    ...
    assume main_~i~5 + 1 <= main_~nlen~5 || !(main_~i~5 + 1 <= main_~nlen~5);
    __VERIFIER_assert_#in~cond := (if main_~i~5 + 1 <= main_~nlen~5 then 1 else 0);
    ...
    assume main_~nlen~5 <= main_~i~5 + 2147483648;
    assume main_~i~5 <= main_~nlen~5 + 2147483647;
    assume 0 < main_~i~5 + 1 || !(0 < main_~i~5 + 1);
    __VERIFIER_assert_#in~cond := (if 0 < main_~i~5 + 1 then 1 else 0);
    havoc __VERIFIER_assert_~cond;
    __VERIFIER_assert_~cond := __VERIFIER_assert_#in~cond;
    assume !(__VERIFIER_assert_~cond == 0);
