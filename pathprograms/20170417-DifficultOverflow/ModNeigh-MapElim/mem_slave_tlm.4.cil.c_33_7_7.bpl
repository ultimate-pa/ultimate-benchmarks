...
modifies ~s_memory1, ~rsp_t_type, ~s_memory3, ~c_empty_req, ~c_read_rsp_ev, ~s_run_i, ~rsp_tt_status, ~c_req_type, ~rsp_t_d, ~c_req_d, ~c_rsp_status, ~req_t_d, ~c_write_req_ev, ~rsp_t_status, ~req_tt_a, ~rsp_t_status___0, ~c_rsp_type, ~req_t_type, ~req_tt_d, ~m_run_i, ~d_t, ~rsp_tt_type, ~s_run_st, ~c_read_req_ev, ~s_memory2, ~c_m_ev, ~c_rsp_d, ~c_req_a, ~req_t_a, ~rsp_t_d___0, ~s_memory0, ~m_run_pc, ~s_run_pc, ~req_t_a___0, ~req_tt_type, ~req_t_d___0, ~c_empty_rsp, ~c_write_rsp_ev, ~rsp_t_type___0, ~a_t, ~rsp_tt_d, ~req_t_type___0, ~m_run_st, ~c_m_lock;
{
  ...
  loc4_0:
    ...
    assume m_run_~a~7 <= 2147483597;
    assume 0 <= m_run_~a~7 + 2147483698;
  ...
  loc5_1:
    ...
    assume !(~s_run_pc == 1);
    ~c_m_ev := 2;
    assume !(m_run_~req_a___0~7 <= 2147483597);
    goto loc9;
  loc6:
    goto loc6_0, loc6_1;
  loc6_0:
    ...
    assume !(~s_run_pc == 1);
    ~c_m_ev := 2;
    assume m_run_~a~7 <= 2147483646;
    assume 0 <= m_run_~a~7 + 2147483649;
