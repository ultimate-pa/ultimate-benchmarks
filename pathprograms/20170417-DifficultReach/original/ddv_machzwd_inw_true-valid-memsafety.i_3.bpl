...
implementation outb(#in~byte : int, #in~port : int) returns (){
    var ~byte : int;
    var ~port : int;
...
procedure outb(#in~byte : int, #in~15 : int) returns ();
modifies ;
...
implementation printk(#in~fmt.base : int, #in~fmt.offset : int) returns (#res : int){
...
implementation init_kernel() returns (){
  ...
  loc6_1:
    assume !!(~i~89 < 10);
    ~shared_workqueue.base, ~shared_workqueue.offset := ~shared_workqueue.base[~i~89 := 0], ~shared_workqueue.offset[~i~89 := 0];
    #t~post79 := ~i~89;
    ~i~89 := #t~post79 + 1;
    havoc #t~post79;
    goto loc5;
  ...
  loc8_1:
    assume !!(~i~89 < 1);
    ~tasklet_registered.tasklet.base, ~tasklet_registered.tasklet.offset := ~tasklet_registered.tasklet.base[~i~89 := 0], ~tasklet_registered.tasklet.offset[~i~89 := 0];
    ~tasklet_registered.is_running := ~tasklet_registered.is_running[~i~89 := 0];
    #t~post80 := ~i~89;
    ~i~89 := #t~post80 + 1;
    havoc #t~post80;
    goto loc7;
...
implementation call_ddv() returns (#res : int){
  ...
  loc9:
    ...
    call init_kernel();
    call #t~ret83 := ##fun~~TO~int(~_ddv_module_init.base, ~_ddv_module_init.offset);
    return;
}
...
implementation ##fun~~TO~int(#in~#fp.base : int, #in~#fp.offset : int) returns (#res : int){
    var #t~ret235 : int;

  loc10:
    call #t~ret235 := zf_init();
    return;
}

procedure ##fun~~TO~int(#in~#fp.base : int, #in~#fp.offset : int) returns (#res : int);
modifies #memory_int, #valid, #length, ~timer_registered.timer.base, ~timer_registered.timer.offset, ~number_timer_registered, ~ddv_ioport_request_start, ~ddv_ioport_request_len, #memory_$Pointer$.base, #memory_$Pointer$.offset, ~number_cdev_registered, ~fixed_cdev_used, ~zf_action, ~action;

implementation zf_init() returns (#res : int){
  ...
  loc11:
    havoc ~ret~75;
    call #t~nondet55.base, #t~nondet55.offset := #Ultimate.alloc(59);
    call #t~ret56 := printk(#t~nondet55.base, #t~nondet55.offset);
    assume -2147483648 <= #t~ret56 && #t~ret56 <= 2147483647;
    havoc #t~nondet55.base, #t~nondet55.offset;
    havoc #t~ret56;
    call #t~ret57 := zf_readw(2);
    return;
}

procedure zf_init() returns (#res : int);
modifies ~zf_action, ~action, #memory_$Pointer$.base, #memory_$Pointer$.offset, #memory_int, #valid, #length, ~timer_registered.timer.base, ~timer_registered.timer.offset, ~number_timer_registered, ~ddv_ioport_request_start, ~ddv_ioport_request_len, ~number_cdev_registered, ~fixed_cdev_used;
...
implementation zf_readw(#in~port : int) returns (#res : int){
    var #t~ret18 : int;
    var ~port : int;
  ...
  loc13:
    ~port := #in~port;
    call outb(~port, 536);
    call #t~ret18 := inw(538);
    return;
}

procedure zf_readw(#in~port : int) returns (#res : int);
...
implementation inw(#in~port : int) returns (#res : int){
  ...
  loc14:
    ~port := #in~port;
    call #t~nondet212.base, #t~nondet212.offset := #Ultimate.alloc(22);
    call __VERIFIER_assert((if ~port % 4294967296 >= ~ddv_ioport_request_start % 4294967296 && ~port % 4294967296 < (~ddv_ioport_request_start + ~ddv_ioport_request_len) % 4294967296 then 1 else 0), #t~nondet212.base, #t~nondet212.offset);
    return;
}

procedure inw(#in~16 : int) returns (#res : int);
modifies #valid, #length;
...
implementation __VERIFIER_assert(#in~expression : int, #in~x.base : int, #in~x.offset : int) returns (){
    var ~expression : int;
...
procedure __VERIFIER_assert(#in~expression : int, #in~x.base : int, #in~x.offset : int) returns ();
modifies ;
