...
implementation __CS_pthread_join(#in~thread : int, #in~value_ptr.base : int, #in~value_ptr.offset : int) returns (#res : int){
  ...
  loc0:
    ...
    assume !(~thread % 256 != 123 && ~thread % 256 > 3);
    assume !(~thread % 256 == 123 || ~__CS_thread_status[~__CS_round % 256,~thread % 256] % 256 == ~__THREAD_RUNNING % 256);
    assume ~__CS_thread_status[~__CS_round % 256,~thread % 256] % 256 == ~__THREAD_UNUSED % 256;
    ~__CS_error := 1;
...
implementation main_thread(#in~arg.base : int, #in~arg.offset : int) returns (#res.base : int, #res.offset : int){
  ...
  loc7:
    ...
    call __CS_cs();
    assume !(~__CS_ret % 256 != 0);
    call #t~ret24 := __CS_pthread_create(~#t2~21.base, ~#t2~21.offset, 0, 0, #funAddr~thread2.base, #funAddr~thread2.offset, 0, 0);
    assume -2147483648 <= #t~ret24 && #t~ret24 <= 2147483647;
    havoc #t~ret24;
    call __CS_cs();
    assume !(~__CS_ret % 256 != 0);
    call #t~mem25 := read~int(~#t1~21.base, ~#t1~21.offset, 1);
    call #t~ret26 := __CS_pthread_join(#t~mem25, 0, 0);
