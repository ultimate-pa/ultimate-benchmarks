...
implementation method1() returns (){
    var ~s1~5 : int;
    var ~s2~5 : int;
    var ~s3~5 : int;
...
modifies ~z_val_t, ~z_req_up, ~comp_m1_st;
...
implementation update_z() returns (){
  loc14:
    assume ~z_val != ~z_val_t;
    ~z_val := ~z_val_t;
    ~z_ev := 0;
    ~z_req_up := 0;
    assume true;
...
procedure update_z() returns ();
modifies ~z_val, ~z_ev, ~z_req_up;
