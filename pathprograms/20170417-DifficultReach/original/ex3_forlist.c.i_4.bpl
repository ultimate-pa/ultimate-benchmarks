...
implementation f(#in~pointer.base : int, #in~pointer.offset : int) returns (){
  ...
  loc4:
    ...
    ~pp.base, ~pp.offset := ~pp.base[~i~5 := ~pointer.base], ~pp.offset[~i~5 := ~pointer.offset];
    ~pstate := ~pstate[~i~5 := 1];
    assume true;
    return;
}
...
implementation g(#in~pointer.base : int, #in~pointer.offset : int) returns (){
    var #t~ite3 : int;
    var #t~post2 : int;
    var ~pointer.base : int, ~pointer.offset : int;
    var ~i~8 : int;
  ...
  loc5:
    ...
    assume true;
    assume !!(~i~8 < 2);
    assume ~pp.base[~i~8] == ~pointer.base && ~pp.offset[~i~8] == ~pointer.offset;
    assume !(~pstate[~i~8] == 1);
    call __blast_assert();
    return;
}

procedure g(#in~pointer.base : int, #in~pointer.offset : int) returns ();
modifies ~pstate;
...
implementation main() returns (#res : int){
  ...
  loc9:
    ...
    call f(~b~12.base, ~b~12.offset);
    call g(~a~12.base, ~a~12.offset);
    return;
}
