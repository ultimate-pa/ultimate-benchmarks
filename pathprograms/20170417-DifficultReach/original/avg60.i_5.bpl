...
implementation avg(#in~x.base : int, #in~x.offset : int) returns (#res : int){
  ...
  loc3_0:
    assume !(~i~3 < 60);
    #res := (if (if ~ret~3 < 0 && ~ret~3 % 60 != 0 then ~ret~3 / 60 + 1 else ~ret~3 / 60) % 4294967296 <= 2147483647 then (if ~ret~3 < 0 && ~ret~3 % 60 != 0 then ~ret~3 / 60 + 1 else ~ret~3 / 60) % 4294967296 else (if ~ret~3 < 0 && ~ret~3 % 60 != 0 then ~ret~3 / 60 + 1 else ~ret~3 / 60) % 4294967296 - 4294967296);
    assume true;
    return;
  loc3_1:
    assume !!(~i~3 < 60);
