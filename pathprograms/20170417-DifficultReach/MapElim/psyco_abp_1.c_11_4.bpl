...
modifies ;
{
  ...
  loc4_1:
    ...
    assume main_~P1~4 == 0;
    main_~method_id~3 := 3;
    assume (main_~P3~4 == 0 && !(main_~this_buffer_empty~3 == 1)) && main_~P3~4 == (if main_~this_expect~3 < 1 && !((main_~this_expect~3 + 1) % 2 == 0) then (main_~this_expect~3 + 1) % 2 - 2 else (main_~this_expect~3 + 1) % 2);
    goto loc5;
  loc5:
    assert false;
}
