...
modifies ;
{
  ...
  loc2_0:
    ...
    assume 2 * main_~sum~5 == main_~n~5 * (main_~n~5 + 1) || !(2 * main_~sum~5 == main_~n~5 * (main_~n~5 + 1));
    __VERIFIER_assert_#in~cond := (if 2 * main_~sum~5 == main_~n~5 * (main_~n~5 + 1) then 1 else 0);
