...
implementation avg(#in~x.base : int, #in~x.offset : int) returns (#res : int){
  ...
  loc1:
    ...
    ~i~3 := 0;
    assume !true;
    #res := (if (if ~ret~3 < 0 && ~ret~3 % 10 != 0 then ~ret~3 / 10 + 1 else ~ret~3 / 10) % 4294967296 <= 2147483647 then (if ~ret~3 < 0 && ~ret~3 % 10 != 0 then ~ret~3 / 10 + 1 else ~ret~3 / 10) % 4294967296 else (if ~ret~3 < 0 && ~ret~3 % 10 != 0 then ~ret~3 / 10 + 1 else ~ret~3 / 10) % 4294967296 - 4294967296);
    assume true;
    return;
}
...
implementation main() returns (#res : int){
  ...
  loc3:
    ...
    ~i~7 := 0;
    assume !true;
    call write~int(~temp~5, ~#x~5.base, ~#x~5.offset + 36, 4);
