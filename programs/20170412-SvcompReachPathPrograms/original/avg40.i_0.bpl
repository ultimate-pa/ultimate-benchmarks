...
implementation avg(#in~x.base : int, #in~x.offset : int) returns (#res : int){
  ...
  loc1:
    ...
    ~i~3 := 0;
    assume !true;
    #res := (if (if ~ret~3 < 0 && ~ret~3 % 40 != 0 then ~ret~3 / 40 + 1 else ~ret~3 / 40) % 4294967296 <= 2147483647 then (if ~ret~3 < 0 && ~ret~3 % 40 != 0 then ~ret~3 / 40 + 1 else ~ret~3 / 40) % 4294967296 else (if ~ret~3 < 0 && ~ret~3 % 40 != 0 then ~ret~3 / 40 + 1 else ~ret~3 / 40) % 4294967296 - 4294967296);
    assume true;
    return;
}
